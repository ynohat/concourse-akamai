...
from akamai.papi import get_property_descriptor, get_property_activation, get_property_rule_tree, get_property_version
...
try:
    ...
    atv = get_property_activation(session, pd.contractId, pd.groupId, pd.propertyId, input.activationId)
    pv = get_property_version(session, pd.contractId, pd.groupId, pd.propertyId, atv.get("propertyVersion"))
    rules = get_property_rule_tree(session, pd.contractId, pd.groupId, pd.propertyId, atv.get("propertyVersion"))
    print(json.dumps(dict(
        version=dict(activationId=input.activationId, propertyVersion=atv.get("propertyVersion")),
        metadata=[
